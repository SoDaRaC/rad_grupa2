import random

x = -10

# print(x > 0)
# print("x je vece od 0")

# if x > 0:
#     print("x je vece od 0")
#     print("ja sam u if bloku koda")
#     print("evo ovde se zavrsava")

# print("ova linija se izvrsava u svakom slucaju...")

# godine = 13

# if godine >= 13:
#     print("korisnik ima vise od 13 godina.")


# vozilo_se_krece = True
# brzina = 40

# if vozilo_se_krece: # ako je vozilo se krece True
#     print("Vozilo se krece...proveri i brzinu")
#     if brzina > 50:
#         print("Prekoracena brzina")
#     if brzina <= 50:
#         print("Brzina je OK.")

proizvod = "Laptop"
cena = 500

# # 1 - prikaz ; 2 - kupi ; 3 - izlaz
# stanje = int(input("Unesite stanje na racunu:"))
# komanda = int(input("Unesite komandu:"))

# if komanda == 1:
#     print("Proizvod:",proizvod,"Cena:", cena)
# if komanda == 2:
#     print("Pristupili ste kupovini.")
#     if stanje >= cena:
#         print("Uspesna kupovina")
#     else:
#         print("Neuspesna kupovina")
# if komanda == 3:
#     print("Izlaz")
# # if komanda != 1 and komanda != 2 and komanda !=3:
# if komanda < komanda 1 or komanda > 3:
#     print("Unesite 1, 2 ili 3. Komanda je pogresna.")

broj = -5

if broj >= 0:
    print("Broj je veci ili jednak 0.") # uslov True
else:
    print("Broj je manji od 0.") # uslov False 

uspesno_logovanje = True

if uspesno_logovanje:
    print("Otvori pocetnu stranu.")
else:
    print("Proverite podatke")

number = 10

if number % 2 == 0:
    print("Broj je paran")
else:
    print("Broj je neparan")

baterija = 85
punjac = False

if baterija > 0 or punjac:
    print("Mozete koristiti telefon.")
else:
    print("Proverite stanje baterije.")

num = 5

if num > 0:
    print("Broj je veci od 0.")
elif num == 0:
    print("Broj je 0.")
elif num == 5:
    print("Broj je tacno 5.")
else:
    print("Broj je manji od 0.")


redniBrojUNedelji = int(input("Unesite redni broj dana u nedelji:"))

if redniBrojUNedelji == 1:
    print("Ponedeljak")
elif redniBrojUNedelji == 2:
    print("Utorak")
elif redniBrojUNedelji == 3:
    print("Sreda")
elif redniBrojUNedelji == 4:
    print("Cetvrtak")
elif redniBrojUNedelji == 5:
    print("Petak")
elif redniBrojUNedelji == 6:
    print("Subota")
elif redniBrojUNedelji == 7:
    print("Nedelja") 
else:
    print("Pogresna vrednost. Unesite izmedju 1 i 7.")


trenutna_vrednost = random.randint(1, 100)
nova_vrednost = int(input("Unesite vrednost: "))

print("Trenutna:", trenutna_vrednost, "Nova:", nova_vrednost)

if nova_vrednost > trenutna_vrednost:
    print("Postigli ste novi high score")
elif nova_vrednost == trenutna_vrednost:
    print("Vrednosti su jednake")
else:
    print("Nova vrednost je manja")


